...
int al_containsAll(ArrayList* pList,ArrayList* pList2)
{
    ...
    else
    {
        ...
        if(pList->size==pList2->size)
        {
            for(i=0;i<pList->size;i++)
            {
                if( (pList+i)->pElements == (pList2+i)->pElements )
               {
                   returnAux=1;
               }
               else
 ...
 *                  - (0) if ok
 */
int al_sort(ArrayList* pList, int (*pFunc)(void* ,void*), int parametro)
{
    int alta=- 1;
    int i,j;
    void* auxplist;

    if(pList== NULL ||pFunc== NULL ||(parametro!= 0&& parametro!= 1))
    {
        return alta;
        }
    ...
    for(i= 0;i< (pList->size -1);i++)
    {
        ...
        {
            if(pFunc(pList->get(pList,i),pList->get(pList,j))== -1 &&parametro== 0)
            {
                auxplist= pList->get(pList,j);

                pList->set(pList,j,pList->get(pList,i)); pList->set(pList,i,auxplist);
            }
            else if(pFunc(pList->get(pList,i),pList->get(pList,j))== 1 &&parametro== 1)
                    {
                        auxplist= pList->get(pList,i);

                        pList->set(pList,i,pList->get(pList,j)); pList->set(pList,j,auxplist);
                    }
        }
    }
